...
def buildHull(points):
    ...
    # Handling the base case for two nodes
    if len(points) == 2:
        ...
        return
    # Handling the base case for three nodes
    if len(points) == 3:
    ...
    # Split the nodes into two parts with half being allocated to the left and the other to the right
    rightNodes = points[len(points) // 2:]
    leftNodes = points[:len(points) // 2]

    # Recursively call BuildHull with the right and left nodes
    buildHull(rightNodes)
    buildHull(leftNodes)
    ...
    # Keep record of nodes moved past during the walk up and down processes
    pastNodes = set()

    # Walk up
    leftSide = leftNodes[-1]
    rightSide = rightNodes[0]

    # Given that the left and rights nodes have room to step up execute this while loop
    # This is the algorithm provided in the week 6 lecture 3 notes
    while (
            turn(leftSide.ccwPoint, leftSide, rightSide) == 1
            or turn(leftSide, rightSide, rightSide.cwPoint) == 1
    ):
        if turn(leftSide.ccwPoint, leftSide, rightSide) == 1:
            pastNodes.add(leftSide)
            leftSide = leftSide.ccwPoint
        else:
            pastNodes.add(rightSide)
            rightSide = rightSide.cwPoint

    # To prevent mutability, save references to new variables once it is done walking upwards
    topLeft = leftSide
    topRight = rightSide
    ...
    # Similar while loop to the walk upwards loop but to check if left or right can step downwards
    while (
            turn(leftSide.cwPoint, leftSide, rightSide) == 2
            or turn(leftSide, rightSide, rightSide.ccwPoint) == 2
    ):
        if turn(leftSide.cwPoint, leftSide, rightSide) == 2:
            pastNodes.add(leftSide)
            leftSide = leftSide.cwPoint
        else:
            pastNodes.add(rightSide)
            rightSide = rightSide.ccwPoint

    # Saving reference nodes for walk down
    bottomRight = rightSide
    bottomLeft = leftSide

    # Merging top and bottom segments to form the perimeter hull
    topLeft.cwPoint = topRight
    topRight.ccwPoint = topLeft
    bottomLeft.ccwPoint = bottomRight
    bottomRight.cwPoint = bottomLeft

    # Remove nodes that interfere with cw/ccw references
    pastNodes -= {topRight, topLeft, bottomRight, bottomLeft}

    # Discard the nodes that are no longer on the hull after each merge.
    # These points are "discarded" by setting their 'cwPoint' and 'ccwPoint' to None
    for node in pastNodes:
        node.cwPoint = None
        node.ccwPoint = None
