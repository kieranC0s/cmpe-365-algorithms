...
def buildTriangles(slice0, slice1):
    ...
    # Brute Force Algorithm for Closet Pair of points.
    # Creating minimum possible distance & minimum possible indicies varaible.
    min_distance, min_index = float('inf'), (0, 0)
    # Creating 2 sets of variables for each slice.
    points1 = slice0.verts
    points2 = slice1.verts
    # Iterate the indexes of the points and vertices of the slices.
    for i1, vert1 in enumerate(points1):
        for i2, vert2 in enumerate(points2):
            # Subtraction to determine the distance between the two points and their coordinates.
            d = length(subtract(vert1.coords, vert2.coords))
            if d < min_distance:
                # Determines New distance and new index values.
                min_distance = d
                min_index = (i1, i2)
    ...
    # Cyclic permutations of the vertices of Slice 1
    cyclic_permutation0 = []
    cyclic_permutation0.extend(points1[min_index[0]:])
    cyclic_permutation0.extend(points1[:min_index[0] + 1])

    # Cyclic permutations of the vertices of Slice 2
    cyclic_permutation1 = []
    cyclic_permutation1.extend(points2[min_index[1]:])
    cyclic_permutation1.extend(points2[:min_index[1] + 1])
    ...
    minArea = [[0 for i in range(len(points1) + 1)]
               for j in range(len(points2) + 1)]
    minDir = [[0 for i in range(len(points1) + 1)]
              for j in range(len(points2) + 1)]
    ...
    for i in range(1, len(cyclic_permutation0)):
        minArea[0][i] = triangleArea(
            cyclic_permutation0[i - 1].coords, cyclic_permutation0[i].coords,
            cyclic_permutation0[0].coords) + minArea[0][i - 1]
    ...
    for j in range(1, len(cyclic_permutation1)):
        minArea[j][0] = triangleArea(
            cyclic_permutation1[0].coords, cyclic_permutation1[j].coords,
            cyclic_permutation1[j - 1].coords) + minArea[j - 1][0]
        minDir[j][0] = Dir.PREV_ROW

    # Fill in the remaining entries of minArea and minDir.  This is very similar to the above, but more general.
    #
    # [2 marks]

    for i in range(1, len(cyclic_permutation1)):
        for j in range(1, len(cyclic_permutation0)):
            # Area left entries
            AL = triangleArea(cyclic_permutation0[j - 1].coords, cyclic_permutation0[j].coords,
                              cyclic_permutation1[i].coords) + minArea[i][j - 1]
            # Area up entries
            AU = triangleArea(cyclic_permutation1[i - 1].coords, cyclic_permutation0[j].coords,
                              cyclic_permutation1[i].coords) + minArea[i - 1][j]
            if AL <= AU:
                minArea[i][j] = AL
                minDir[i][j] = Dir.PREV_COL
            else:
                minArea[i][j] = AU
                minDir[i][j] = Dir.PREV_ROW
    ...
    triangles = []
    r = len(cyclic_permutation1) - 1
    c = len(cyclic_permutation0) - 1

    while r >= 0 and c >= 0:
        if minDir[r][c] == Dir.PREV_ROW:
            Vertss = cyclic_permutation1[r], cyclic_permutation0[c], cyclic_permutation1[r - 1]
            tri = Triangle(Vertss)
            triangles.append(tri)
            r = r - 1
        else:
            Vertss = cyclic_permutation1[r], cyclic_permutation0[c], cyclic_permutation0[c - 1]
            tri = Triangle(Vertss)
            triangles.append(tri)
            c = c - 1
...
def display(wait=False):
    ...
    if labelEdges:
        ...
        for slice in slicesToDraw:
            for vert in slice.verts:
                drawText(scalarMult(0.5, add(vert.coords, vert.nextV.coords)),
                         ('%s-%s' % (repr(vert), repr(vert.nextV))))

    if labelTris:
        glColor3f(0, 0, 0)
        for tri in allTriangles:
            drawText(scalarMult(0.3333, add(tri.verts[0].coords, add(tri.verts[1].coords, tri.verts[2].coords))),
                     repr(tri))
...
def actOnMouseMovement(window, button, x, y):
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        ...
        if abs(rotationAngle) < 0.0001:
            ...
            d = math.sqrt(
                rotationAxis[0] * rotationAxis[0] + rotationAxis[1] * rotationAxis[1] + rotationAxis[2] * rotationAxis[
                    2])
